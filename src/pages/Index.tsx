...
export function IndexPage() {
  ...
  const [backendStatus, setBackendStatus] =
    useState<HealthStatus>('unreachable')
  ...
  const handleAnalyze = async () => {
    ...
    if (faceImage && faceImage.size > maxSize) {
      toast.error(
        `Face image is too large (${(
          ...
          1024 /
          1024
        ).toFixed(1)}MB). Maximum size is 5MB.`,
      )
      return
    }
    if (xrayImage && xrayImage.size > maxSize) {
      toast.error(
        `X-ray image is too large (${(
          xrayImage.size /
          1024 /
          1024
        ).toFixed(1)}MB). Maximum size is 5MB.`,
    ...
    try {
      ...
      const results = await postPredict(formData, true)
      navigate('/results', { state: { results, healthDetails } })
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Analysis failed'
      if (message.includes('{"ok":false')) {
        try {
          const errorData = JSON.parse(
            message.substring(message.indexOf('{"ok":false')),
          )
          toast.error(
            `Analysis failed: ${
        ...
        toast.error(
          'Backend connection failed. Please check if the backend server is running on port 8000.',
        )
        setShowDiagnostics(true)
      } else {
        toast.error(`Analysis failed: ${message}`)
          ...
          <div className="max-w-4xl mx-auto text-center py-12">
            ...
            <Reveal delay={0.2}>
              <p className="text-xl md:text-2xl text-[#64748B] mb-12 max-w-3xl mx-auto leading-relaxed font-medium">
                Advanced AI technology for early PCOS detection using facial
                recognition and X-ray analysis. Upload your images for instant,
            ...
            <Card className="shadow-vibrant-lg border-0 card-gradient backdrop-blur-sm">
              <CardHeader className="text-center pb-6">
                ...
                <CardDescription className="text-lg max-w-2xl mx-auto text-slate-600">
                  Upload a facial photo and/or uterus X-ray for comprehensive
                  PCOS analysis. You can upload one or both images.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-8">
                ...
                {backendStatus === 'unreachable' && (
                  <Alert className="border-orange-200 bg-orange-50">
                    ...
                    <AlertDescription>
                      Backend is unreachable. If running in a sandbox
                      environment, please set up a public backend URL using the
                      "Set API URL" button above.
                    </AlertDescription>
                  </Alert>
                )}
                ...
                <div className="text-center space-y-4">
                  <Button
                    ...
                    disabled={
                      !hasImages || isAnalyzing || backendStatus !== 'online'
                    }
                    size="lg"
            ...
            <div className="text-center space-y-12">
              <div>
                ...
                <p className="text-xl text-slate-600 max-w-2xl mx-auto">
                  Our AI system uses advanced deep learning models to analyze
                  multiple indicators
                </p>
              </div>
                ...
                {[
                  {
                    ...
                    description:
                      'AI analyzes facial features for visible PCOS symptoms like hirsutism and skin changes',
                    delay: 0,
                  ...
                  },
                  {
                    ...
                    description:
                      'YOLOv8 object detection identifies morphological features in uterus X-rays',
                    delay: 0.1,
                  ...
                  },
                  {
                    ...
                    description:
                      'Intelligent fusion of both analyses provides comprehensive risk assessment',
                    delay: 0.2,
                  ...
                  <Reveal key={item.title} delay={item.delay}>
                    <Card className="h-full card-gradient hover:shadow-vibrant-lg transition-all duration-300 hover:-translate-y-2 hover:scale-105 border-0">
                      <CardContent className="p-8 text-center">
                        <div
                          className={`inline-flex items-center justify-center w-16 h-16 rounded-full bg-gradient-to-br ${item.color} mb-6 shadow-vibrant`}
                        >
                ...
                {[
                  {
                    ...
                    answer:
                      'Our AI models are trained on extensive medical datasets and achieve high accuracy rates. However, this tool is for screening purposes only and should not replace professional medical diagnosis. Always consult with healthcare professionals for definitive diagnosis.',
                  },
                  {
                    question: 'Is my data secure and private?',
                    answer:
                      'Yes, your privacy is our top priority. Images are processed locally when possible, and any data transmitted is encrypted. We do not store your images permanently, and all processing is done for analysis purposes only.',
                  },
                  {
                    question: 'What types of images should I upload?',
                    answer:
                      'For facial analysis, upload a clear, front-facing photo with neutral expression and good lighting. For X-ray analysis, use standard pelvic X-ray images with good contrast. Supported formats include JPEG, PNG, and WebP.',
                  },
                  {
                    question: 'How should I interpret the results?',
                    answer:
                      'Results are presented as risk assessments (Low, Moderate, High) based on AI analysis. These are screening indicators only and should be discussed with a qualified healthcare professional for proper medical interpretation.',
                  },
                  {
                    question: 'Can I use smartphone photos?',
                    answer:
                      'Yes, smartphone photos can be used for facial analysis if they are clear, well-lit, and show the face directly. However, professional medical imaging is recommended for X-ray analysis for best results.',
                  },
                  {
                    question: 'What image formats are supported?',
                    answer:
                      'We support JPEG, PNG, and WebP image formats. The system automatically handles image orientation and optimization for analysis.',
                  },
                  {
                    question: 'Will my images be stored?',
                    answer:
                      'No, your images are not permanently stored. They are processed for analysis and then discarded. We prioritize your privacy and data security.',
                  },
                  {
                    question: 'Does this replace lab tests?',
                    answer:
                      'No, this tool is for screening and educational purposes only. It does not replace laboratory tests, clinical examinations, or professional medical diagnosis. Always consult healthcare professionals for comprehensive evaluation.',
                  },
                  {
                    question: 'What should I do after a high-risk result?',
                    answer:
                      'If you receive a high-risk assessment, consult with a qualified healthcare professional immediately. They can perform proper diagnostic tests and provide appropriate medical guidance based on your individual situation.',
                  },
                ].map((faq) => (
                  <AccordionItem
                    key={faq.question}
                    value={faq.question}
                    className="card-gradient border-0 rounded-lg px-6 shadow-vibrant"
                  >
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3 text-sm">
              ...
              <ol className="list-decimal list-inside space-y-1 ml-4">
                <li>Start your backend locally</li>
                <li>
                  Expose it publicly with ngrok:{' '}
                  <code className="bg-muted px-1 rounded">ngrok http 8000</code>
                </li>
              ...
              <p className="mt-4">
                ...
                <code className="bg-muted px-1 rounded">
                  {resolveApiBase() || window.location.origin}/health
                </code>{' '}
                in your browser - it should return JSON with status "ok".
              </p>
            </div>
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto text-center space-y-6 relative z-10">
            ...
            <div className="space-y-2 text-slate-200">
              ...
              <p className="text-sm">
                Project by <span className="font-medium text-emerald-400">DHANUSH RAJA (21MIC0158)</span>
              </p>
            </div>
          </div>
        </div>
